...
## [m2] — 2026-06-12 — Content & Director

### Added
- **Все 16 аномалий Appendix A** (в брифе заявлено «15», в списке 16 — D-011):
  + A2 ящик переполнен письмами, A3 велосипед исчез (велосипед+замок+стояк —
    теперь база каждой площадки), B2 потолок −15% (фальшпотолок+лампа ниже),
    B3 дубль двери №36, C2 мерцание-сердцебиение (строб-класс), C3 пульс света
    из-под двери, D2 зеркальный трафарет (гейт этажей 1/8 — D-012),
    E1 глазок темнеет при приближении, E2 силуэт уходит от взгляда (>0.4 c,
    единственная T3), F1 эхо шагов не в такт (через AudioDirector, teardown).
- **DirectorMath** (чистый класс, GUT): кривая base 6+7.5·(этаж−1), гейты
  тиров (T3 — финальная треть), микс эмбиента, условия скер-слотов.
- **Director M2**: relief valves (≥2 спокойных пролёта после сброса/T3),
  silence events (1–2 на забег, 5–10 c, hard-mute Ambience), idle-вход
  («дом смотрит»), шедулинг скер-слотов, режим фоточувствительности
  (строб-аномалии исключаются селектором).
- **Скер-слот 1**: силуэт «ближе, чем позволяет физика» + стингер + микрошейк
  камеры (≤0.3 c) + глитч. **Скер-слот 2**: на этаже 8 лампа за спиной гаснет
  (клик) + дыхание в затылок.
- **Финал «Дом» (скелет)**: дверь №36 приоткрыта (CSG-проём, тёмная прихожая,
  тёплый отсвет), подход → заморозка → лампа гаснет → карточка.
- **3-слойный эмбиент** по Tension (PLACEHOLDER_amb_l1/l2/l3) + стингер +
  щелчок лампы; play_varied с volume_db; глазки/велосипед/стояк в базе модуля.
- Dev: `ETAZH9_FORCE=<id>` — форс аномалии; smoke = полный честный забег
  1→9 до концовки (интеграционный тест DoD M2).
- GUT: 28 тестов (математика Director'а, гейты тиров/строба селектора).

### Fixed
- **D-010**: фантомные body_entered от перемещённых/перестроенных Area3D после
  re-anchor — кулдаун 5 физ. кадров на коммиты + deferred-мониторинг новых
  area + коммит только с уровней ±1.
- Перила марша укорочены: нижний хвост больше не нависает над площадкой
  предыдущего модуля (упирался в голову капсулы).
